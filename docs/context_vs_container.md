...
export const userRegistratorFactory = (context: { repository: UserRepository }) => {};
```
...
const configureRouter = (app: core.Express, diContainer: IDIContainer) => {
  ...
  app.route('/users').get(usersController.actionIndex).post(usersController.actionCreate);
};
