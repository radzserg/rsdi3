...
- **Two different Node versions, on purpose.** `engines.node >=16.9.0` is what the _published_ package needs at runtime; `devEngines.runtime >=22` and `.nvmrc` (26) are what _contributing_ needs. They are unrelated audiences, so the mismatch is correct — don't "fix" it by aligning them. `devEngines` is enforced by npm 11+ and pnpm 11 when installing this repo and ignored when the package is consumed as a dependency.
  ...
  Its value is constrained from both sides, and getting it wrong breaks installs in a way that looks nothing like a version problem. It must be **at or below** the lowest entry in the CI test matrix, or the matrix's own `pnpm install` fails. It must also be a **subset of the `engines` range that oxfmt, oxlint, and rolldown declare for their native bindings** (`^20.19.0 || >=22.12.0` today) — those bindings are optional dependencies, and pnpm skips an optional dependency unless every version in the declared range satisfies it. `>=22` looks harmless but admits 22.0–22.11, so pnpm silently drops the platform binding and the build dies with `Cannot find native binding` / `Cannot find module '@oxfmt/binding-linux-x64-gnu'`. Hence `>=22.12.0`. A local install won't reveal this if `node_modules` already exists — reproduce with a clean install in a container.
