...
- **The runtime floor is `engines.node >=16.9.0`, and it is load-bearing.** That exact number comes from `Object.hasOwn`, which `DIContainer` uses in four places and which landed in Node 16.9 — not 16.0. Before reaching for any newer built-in, check its Node availability and raise `engines` in the same change, or you silently break consumers below the new floor. This has already gone wrong once: 3.1.1 shipped `Object.hasOwn` with `engines` still unset. The `min-node` CI job guards this: it builds the package and imports it under a `node:<floor>-alpine` container, with the tag derived from `engines.node` so the check always matches the declaration. The script is `scripts/smoke-min-node.mjs` — keep it to the public API and plain assertions, since it runs on a Node too old for Vitest.
